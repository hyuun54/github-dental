...
public class FrontController extends HttpServlet{
	...
	private String uploadedPath = null; //업로드가 되는 실제 경로
	...
	protected void doProcess(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (command==null) {
			...
			if (multi != null) {
				//<form method="post" enctype="multipart/form-data"> 경우
				// command = request.getParameter("command"); 작동 안함
				command = multi.getParameter("command");
				
				// delete_image : 이전에 업로드 했던 이미지 입니다.
				//이 파라미터는 상품 수정 페이지 에서 넘어 옵니다.
				String delete_image = multi.getParameter("delimage");
