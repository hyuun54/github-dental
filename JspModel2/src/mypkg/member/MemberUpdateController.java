...
public class MemberUpdateController extends SuperClass{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		bean.setSalary(Integer.parseInt(request.getParameter("salery")));
	...
	public boolean validate(HttpServletRequest request) {
		...
		if (bean.getId().length() < 4 || bean.getId().length() > 10) {
			request.setAttribute(super.PREFIX + "id", "아이디는 4자리 이상 10자리 이하이여야 합니다.");
			isCheck = false;
		}
		...
		//날짜는 yyyy/MM/dd 또는 yyyy-MM-dd 형식(정규 표현식) 
		String regex = "\\d{4}[-/]\\d{2}[-/]\\d{2}" ;
		if( bean.getHiredate() == null){
			bean.setHiredate( "" );
		}
		//util.regex.pattern import
