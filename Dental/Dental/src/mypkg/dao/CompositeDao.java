...
public class CompositeDao extends SuperDao{
	public List<Postcode> SelectDataZipcode(String dong) {
		...
		sql += " search_word like '%" + dong + "%' or"  ;
		sql += " bd_ma_sn like '%" + dong + "%' "  ;
	...
	public List<Combo01> View01() {
		...
		sql += " on m.id=b.writer "; //복사할 때는 ; 조심
	...
	public List<Combo02> View02() {
		...
		sql += " group by m.name ";
		sql += " order by m.name desc "; //복사할 때는 ; 조심
	...
	public List<Combo03> View03() {
		...
		sql += " order by p.name desc, m.name asc "; //복사할 때는 ; 조심
